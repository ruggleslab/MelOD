...
selection_list_server <- function(dds, id, shared_reactives,clinical_data = NULL) {
  ...
  #' @description Handles selection for list input, updating the shared reactive variables based on the user's selection.
  #' @param dds A list of DESeq2 datasets for different treatment types.
  #' @param clinical_data A list of clinical data frames corresponding to each DESeq2 dataset. Default is NULL.
  #' @param id A unique module ID used to identify the module in the Shiny application.
  #' @param shared_reactives A reactiveValues object for sharing reactive variables across modules.
  ...
  moduleServer(id, function(input, output, session) {
    observe({
      ...
      if (!is.na(selection_index) && selection_index >= 1 && selection_index <= length(dds)) {
        ...
        # Set selected_clinical_data based on the availability of clinical_data
        selected_clinical_data <- NULL
        if (!is.null(clinical_data) && length(clinical_data) >= selection_index) {
          selected_clinical_data <- clinical_data[[selection_index]]
        }
...
selection_server <- function(dds, id, clinical_data = NULL) {
  ...
  moduleServer(id, function(input, output, session) {
    selected_dds <- reactiveVal()
    selected_clinical_data <- reactiveVal(NULL) # Initialize to NULL in case clinical_data is not provided
    ...
    observe({
      if (length(dds) > 1) {
        # Check if clinical_data is not NULL and has the same length as dds
        if (!is.null(clinical_data) && length(clinical_data) == length(dds)) {
          selection_list_server(dds, id, shared_reactives, clinical_data)  # Pass shared_reactives if clinical_data is available
        } else {
          selection_list_server(dds, id, shared_reactives, NULL)  # Pass NULL if clinical_data is not available
        }
      } else {
        selected_dds(dds[[1]])  # Default to the first dataset if only one is available

        # Set clinical data only if it is provided and has a valid first element
        if (!is.null(clinical_data) && length(clinical_data) > 0) {
