sc_violin_server <- function(id, shared_reactives) {
  #' Single-Cell Violin Plot Server Module
  #'
  #' This function defines a Shiny module server for generating single-cell violin plots.
  #' It handles the setup of input options for X and Y axes, processes the data for violin
  #' plot visualizations, and provides event handling for displaying additional information
  #' through alerts. The module supports both cell information and gene expression data.
  #'
  ...
  moduleServer(id, function(input, output, session) {
    ...
    processed_data <- reactive({
      if (input$choice_comparison_violin == "Cell info") {
        ...
        violin_y_axis <- input$violin_plot_Y_gene
      }
