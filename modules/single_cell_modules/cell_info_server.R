cell_info_server <- function(id, shared_reactives) {
  moduleServer(id, function(input, output, session) {
    ...
    processed_data <- reactive({
      ...
      process_cell_data(shared_reactives$sc1conf_data(), shared_reactives$sc1meta_data(), input$cell_plot_culstered_X_axis, 
    ...
    output$cell_plot_culstered <- renderPlotly({
      ...
      p1 <- cell_plotly(processed_data(), debounced_marker_size(),input$cell_plot_culstered_X_axis, 
                        input$cell_plot_culstered_Y_axis, input$cell_plot_culstered_color, input$cell_plot_culstered_label)
      
      if (input$split_view) {
        p2 <- cell_plotly(processed_data(), debounced_marker_size(),input$cell_plot_culstered_X_axis, 
